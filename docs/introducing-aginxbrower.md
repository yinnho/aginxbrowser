# AginxBrower：让 AI Agent 拥有一颗"轻量浏览器心脏"
...
**最硬核的一点**：微信公众号文章，AginxBrower **不需要任何 cookie、不需要登录态**就能抓到完整正文。

原因是它有 stealth 指纹（TLS 层伪装成 Chrome）+ 一致的 navigator 指纹（UA / platform / language 自洽）。微信的风控只看"你像不像真浏览器"，AginxBrower 装得够像，就放行了。

## 一个反直觉的设计：它是"外挂"，不是"库"

大多数抓取方案是"把浏览器嵌进你的代码"——`pip install playwright`，然后 `from playwright.sync_api import sync_playwright`。

AginxBrower 反过来：**它是一个独立服务，挂在系统里，谁需要谁调**。
...
2. **宿主零改动**。以 OpenCarrier（我们的 Agent 运行时）为例——它的内置 `web_fetch` 工具默认走 reqwest。接入 AginxBrower 只需要一个环境变量：

   ```bash
   export AGINXBROWER_URL=http://127.0.0.1:8089
   ```

   设了，`web_fetch` 遇到微信这类风控站就自动转给 AginxBrower；没设，行为完全不变，可随时回退。**Agent 的接口、参数、语义都不用改**，Agent 根本不知道浏览器存在。

3. **失败降级**。AginxBrower 挂了？`web_fetch` 自动回退到 reqwest，整个系统不瘫。

这种"外挂基础设施"的定位，让 AginxBrower 可以无痛接入任何已有的 Agent / 爬虫 / RAG 系统，而不用重写它们的抓取层。

## 它怎么做到这么轻？

关键决策：**不实现完整的浏览器，只实现"够用的浏览器"**。

| 组件 | Chromium | AginxBrower |
|------|----------|-------------|
| JS 引擎 | V8 | V8（同一个） |
...
开发 AginxBrower 的过程里，有几个反直觉的发现，值得分享：
...
```bash
git clone https://github.com/yinnho/aginxbrower
cd aginxbrower
cargo build --release --features stealth   # 含指纹伪装
./target/release/aginxbrower               # 监听 :8089
...
仓库：**https://github.com/yinnho/aginxbrower**
文档：README（API 全字段）+ `docs/ANTI_BOT.md`（反爬实战与架构权衡）

## 写在最后

AginxBrower 不是一个"Chromium 杀手"，它是一个**务实的中间态**——在"reqwest 太弱"和"Chromium 太重"之间，给 Agent 一个能用的浏览器内核。
...
*AginxBrower 是 [OpenCarrier / Aginx](https://github.com/yinnho) 生态的一部分——Agent 互联网基础设施，访问 Agent 就像访问网站一样简单。*
