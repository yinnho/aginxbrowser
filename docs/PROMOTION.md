...
我们 fork 了 Blitz，一行 patch 修掉——当网络层是空操作时，不把 stylesheet 标记成"阻塞渲染的关键资源"。修完，百度从 1 种颜色（全白）变成 1653 种颜色，搜索框、结果列表全出来了。**PR 已被上游合并（DioxusLabs/blitz#636），fork 已退役，现在直接跟踪上游。**
