...
| `session_network` | Read the session's network request log; `filter: "media"` extracts playback/stream URLs (m3u8, mp4, ...) actually requested by the page — the reliable way to get a real video link. `include_bodies: true` adds an `xhr` array with the page's script-initiated response bodies (its own API face), narrowed by `url_contains`. The payload also carries `in_flight` (url/method/age_ms) when script-initiated requests are dispatched but not yet settled — the "still executing?" check before re-issuing a save |
