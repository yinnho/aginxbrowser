# AginxBrower 反爬能力与站点实战

记录 AginxBrower 对各类反爬站点的实测结论、根因分析、以及未来的改进方向。
...
AginxBrower 的定位（V8 + 内联 Obscura 自研 DOM + wreq stealth）覆盖 2/4，部分覆盖 1（代理）。
...
- `__zse_ck` 首次获取需过易盾验证码（aginxbrower 不会自动过，人工一次后长期有效）
...
- `d_c0` 首次靠知乎 JS 在客户端生成；aginxbrower 访问 `zhihu.com/signin` 能自动生成 d_c0（zse_ck JS 跑通），证明 V8+stealth 足以执行知乎风控 JS
...
两条出路（研究结论）：
1. **外挂真 Chromium via CDP**（推荐）：aginxbrower 变编排层，内核用真 Chrome（白拿 TLS/DOM/V8 指纹），Obscura/V8 专职跑签名脚本。对标 nodriver
