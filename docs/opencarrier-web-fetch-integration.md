# OpenCarrier web_fetch 接入 AginxBrower（外挂定位）

> 本文是 OpenCarrier `web_fetch` 接入 AginxBrower 的实际落地方案。AginxBrower 侧无需改动。

## 定位

AginxBrower 是**纯外挂基础设施**——像真实浏览器一样挂在系统里，`web_fetch` 在内部按需调用它，对**所有 agent 完全透明**。agent 还是调 `web_fetch(url)`，参数、返回、语义都不变；`web_fetch` 自己决定走 reqwest 直连还是走 AginxBrower。
...
- **失败降级**：AginxBrower 调用失败/空内容时回退 reqwest，不报错

## AginxBrower 是什么

独立 HTTP 服务（systemd 守护 `aginxbrower.service`），默认 `127.0.0.1:8089`（仅本机可达，公网被防火墙拦）。提供：
...
源码与文档：https://github.com/yinnho/aginxbrower
...
```rust
/// 外挂 AginxBrower 需要兜底抓取的站点（JS 渲染或风控）。
...
if method_upper == "GET" && should_use_aginxbrower(url) && aginxbrower_url().is_some() {
    if let Ok(content) = self.fetch_via_aginxbrower(url).await {
        ...
        let result = format!("HTTP 200 (via AginxBrower)\n\n{}",
...
### 调 AginxBrower（请求格式对齐 `browser.rs` 的 `do_fetch_request`）

```rust
async fn fetch_via_aginxbrower(&self, url: &str) -> Result<String, String> {
    let base = aginxbrower_url().unwrap();
...
`web_fetch.rs` 第一步的 `ssrf::check_ssrf(url)` 校验的是**目标 URL**（如微信公网域名），合法通过。AginxBrower 跑在 `127.0.0.1:8089`，`web_fetch` 对它发的 plain reqwest POST 是固定可信服务地址，不经 SSRF check（与连任何外部 API 同理）。AginxBrower 服务端自身也有 SSRF 防护，双层保险。

## 行为矩阵

| `AGINXBROWER_URL` | URL 域名 | 走哪 | 行为 |
|-------------------|----------|------|------|
| 设了 | mp.weixin.qq.com 等 | AginxBrower | ✅ 拿到正文 |
...
- `web_fetch` 内嵌 AginxBrower —— 增强抓取，对 agent 透明（agent 不知道浏览器存在）
...
3. AginxBrower 已 systemd 守护，开机自启 + 崩溃重启

## 验证

- 不设 `AGINXBROWER_URL`：所有 `web_fetch` 行为与改造前一致（回归）
- 设了：抓微信文章 → 走 AginxBrower 拿到正文；抓 JSON API → 走 reqwest，速度正常
- 停掉 AginxBrower：风控站请求回退 reqwest，不报错
- 同一微信 URL 第二次抓 → 走 cache

## 不采用的替代方案

- **全量替换**（所有 web_fetch 走 AginxBrower）：API/JSON 抓取变慢，服务挂了全瘫
