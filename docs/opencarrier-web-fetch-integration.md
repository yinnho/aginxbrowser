...
- **唯一开关**：环境变量 `AGINXBROWSER_URL`。不设 = 不启用 = 纯 reqwest（行为等同改造前，完全可回退）
...
/// 读 AGINXBROWSER_URL。未设/空 → None（不启用，纯 reqwest）。
...
fn should_use_aginxbrowser(url: &str) -> bool {
    ...
    AGINXBROWSER_HOSTS.iter().any(|h| host.contains(h))
}
```
...
env var 差异：`browser.rs` 未设 `AGINXBROWSER_URL` 时用默认 `127.0.0.1:8089`（默认启用）；`web_fetch` 未设时为 None（默认禁用，保回归）。各自语义合理。
...
3. **新增** 一个调 AginxBrowser `/search` 的内置工具（或复用 web_fetch 的 AginxBrowser
   路由），例如 `web_search`：
   - `web_search(q, fetch_top=0)` → POST `http://<AGINXBROWSER_URL>/search`
   - 复用 `AGINXBROWSER_URL` 环境变量（和 web_fetch 同一个开关）
...
2. OpenCarrier 启动环境设 `export AGINXBROWSER_URL=http://127.0.0.1:8089`
3. AginxBrowser 已 systemd 守护，开机自启 + 崩溃重启

## 验证

- 不设 `AGINXBROWSER_URL`：所有 `web_fetch` 行为与改造前一致（回归）
