...
fn storage_file(origin: &str) -> Option<std::path::PathBuf> {
    if crate::env_knobs::ephemeral() {
        return None;
    }
    let dir = std::env::var("AGINXBROWSER_STORAGE_DIR")
        .ok()
        .or_else(|| std::env::var("AGINXBROWSER_COOKIE_STORE_DIR").ok())
        .or_else(|| crate::env_knobs::app_data_dir().map(|p| p.to_string_lossy().into_owned()))
...
// ---------------------------------------------------------------------------
// WebCrypto ECDSA (P-256 / P-384). The key material travels between the JS
// shim and these ops as a flat payload of four u16-BE length-prefixed
...
fn ecdsa_flat(parts: [&[u8]; 4]) -> Vec<u8> {
...
fn ecdsa_digest(hash: &str, data: &[u8]) -> Result<Vec<u8>, deno_error::JsErrorBox> {
    use sha1::Digest as _;
    Ok(match hash {
        "SHA-1" => sha1::Sha1::digest(data).to_vec(),
        "SHA-256" => sha2::Sha256::digest(data).to_vec(),
...
#[op2]
#[buffer]
fn op_subtle_ecdsa_generate(#[string] curve: &str) -> Result<Vec<u8>, deno_error::JsErrorBox> {
    ecdsa_curves!(curve, {
...
#[op2]
#[buffer]
fn op_subtle_ecdsa_import_private(
    #[string] curve: &str,
    is_scalar: bool,
    #[buffer] material: &[u8],
) -> Result<Vec<u8>, deno_error::JsErrorBox> {
...
#[op2]
#[buffer]
fn op_subtle_ecdsa_import_public(
    #[string] curve: &str,
    #[buffer] material: &[u8],
) -> Result<Vec<u8>, deno_error::JsErrorBox> {
    ecdsa_curves!(curve, {
...
pub fn build_extension() -> Extension {
    Extension {
        ...
        ops: std::borrow::Cow::Owned(vec![
            ...
            op_subtle_pbkdf2(),
            op_subtle_hkdf(),
            op_subtle_ecdsa_generate(),
            op_subtle_ecdsa_import_private(),
            op_subtle_ecdsa_import_public(),
            op_subtle_ecdh_derive_bits(),
            op_subtle_ecdsa_sign(),
            op_subtle_ecdsa_verify(),
            op_random_bytes(),
            op_url_parse(),
            op_url_set(),
            op_url_resolve(),
