...
#[tokio::test(flavor = "current_thread")]
    async fn fetch_parked_evaluate_falls_through_after_resolution_timeout() {
        ...
        impl Drop for RestoreTimeout {
            fn drop(&mut self) {
                diting::diting_js::ops::INTERCEPT_RESOLUTION_TIMEOUT_MS
                    .store(10_000, std::sync::atomic::Ordering::Relaxed);
            }
        }
        let _restore = RestoreTimeout;
        diting::diting_js::ops::INTERCEPT_RESOLUTION_TIMEOUT_MS.store(300, std::sync::atomic::Ordering::Relaxed);
...
<div style=\"height:500px;background:rgb(0,0,255)\"></div>\
    ...
    #[cfg(feature = "screenshot")]
    fn decoded_frame(resp: &crate::cdp::types::CdpResponse) -> image::DynamicImage {
...
r.addEventListener('change',function(){globalThis.__ev.push('change:'+r.value)});</script>\
    ...
    fn spawned_navigate_resolves_at_commit_and_tail_emits_lifecycle() {
        ...
        tokio::task::LocalSet::new().block_on(&rt, async {
            ...
            crate::cdp::domains::page::emit_navigation_tail(
                &mut ctx,
                &pending,
                &page_id,
                None,
        ...
        tokio::task::LocalSet::new().block_on(&rt, async {
            ...
            for _ in 0..500 {
                    ...
                    .find(|e| e.method == "Fetch.requestPaused")
                {
                    ...
                    let same_id = |e: &crate::cdp::types::CdpEvent| {
