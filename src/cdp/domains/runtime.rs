use diting::diting_js::runtime::{EvalOutcome, ExceptionInfo, RemoteObjectInfo};
    ...
    match method {
        "enable" => {
            ...
            if let Some(page) = ctx.get_session_page(session_id) {
                let event = crate::cdp::types::CdpEvent {
            ...
            let outcome = {
                ...
                match tokio::time::timeout(
                    ...
                    page.evaluate_for_cdp_outcome(
                        ...
                        return_by_value,
                        await_promise,
                        diting::diting_js::runtime::DEFAULT_AWAIT_BUDGET_MS,
                        frame_nid,
                    ),
                )
                .await
