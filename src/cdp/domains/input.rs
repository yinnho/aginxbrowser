...
pub const INPUT_HELPERS: &str = r#"(function() {
...
pub fn mouse_button_code(button: &str) -> u8 {
    match button {
        "middle" => 1,
        "right" => 2,
        "back" => 3,
...
pub fn mouse_button_mask(button: &str) -> u64 {
    match button {
        "right" => 2,
        "middle" => 4,
        "back" => 8,
...
pub fn mouse_move_js(x: f64, y: f64, buttons: u64, modifiers: u64) -> String {
    let (alt_key, ctrl_key, meta_key, shift_key) = modifier_flags(modifiers);
    format!(
        "(function() {{\
            var hit = globalThis.__diting_hitTarget ? globalThis.__diting_hitTarget({x},{y}) : null;\
