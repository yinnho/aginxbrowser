...
pub(crate) fn pptx_package(
    ...
    zip.add(
        ...
        // The master carries BOTH relationships real decks have: its layouts
        // and the theme. ECMA-376 makes the theme rel part of the slide
        // master's contract — WPS sends packages without it to the repair
        // path (repair drops slides), so this is not optional polish.
        format!(
            r#"{XML_DECL}<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideLayout" Target="../slideLayouts/slideLayout1.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme" Target="../theme/theme1.xml"/></Relationships>"#
        )
        .as_bytes(),
    );
...
mod tests {
    ...
    fn pptx_package_shape() {
        ...
        // The master must relate to the theme (WPS repair trigger when
        // missing) and carry txStyles — both checked against a real
        // WPS-authored golden package.
        let master_rels = entries
            .iter()
            .find(|(n, _)| n == "ppt/slideMasters/_rels/slideMaster1.xml.rels")
            .map(|(_, d)| String::from_utf8_lossy(d).to_string())
            .expect("master rels");
        assert!(master_rels.contains("theme/theme1.xml"), "master relates to theme");
        let master = entries
            .iter()
            .find(|(n, _)| n == "ppt/slideMasters/slideMaster1.xml")
            .map(|(_, d)| String::from_utf8_lossy(d).to_string())
            .expect("master");
        assert!(master.contains("<p:txStyles>"), "master carries txStyles");
