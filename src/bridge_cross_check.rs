    ...
    use diting::diting_css::{self, ComputedStyle, ParsedRule};
    use diting::diting_dom::tree::{DomTree, NodeId};
    use diting::diting_layout::*;
    ...
    fn our_styles(tree: &DomTree, rules: &[ParsedRule]) -> HashMap<NodeId, ComputedStyle> {
        compute_styles(tree, rules, (VW, VH))
    }
    ...
    /// scripts/make_font_fixture.py. The files live with the engine
    /// (diting_layout's own tests shape the same bytes) — both sides of the
    /// cross-check must read one source of truth, so this reaches across
    /// the workspace split instead of forking the binaries.
    const FIXTURE_REGULAR: &[u8] = include_bytes!("../crates/diting/src/diting_layout/fixtures/diting-fixture-regular.ttf");
    const FIXTURE_BOLD: &[u8] = include_bytes!("../crates/diting/src/diting_layout/fixtures/diting-fixture-bold.ttf");
    ...
    fn baseline_model_is_parley_quantized() {
        use diting::diting_layout::text::baseline_offset;
    ...
    fn float_continuation_narrows_nested_blocks() {
        ...
        let sheet2 = r#"
            ...
            #deep { height: 20px; }
        "#;
        let tree2 = diting::diting_dom::tree_sink::parse_html(html2);
    ...
    fn srcset_parse_rejects_bad_entries_and_keeps_good_neighbors() {
        use diting::diting_layout::image::{parse_srcset, select_srcset_candidate};
