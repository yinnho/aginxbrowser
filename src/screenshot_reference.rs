//! The reference render pipeline (DioxusLabs/blitz, rev 2fa6434d) — the
//! engine diting grew up against. The module is named by purpose, not by
//! vendor: it exists as the comparison oracle, and the fact that the oracle
//! is Blitz is lineage (credited here and in Cargo.toml), not identity.
//! Gated behind `blitz-reference` and NOT
//! compiled into production/device builds (those stop at the `screenshot`
//! feature = diting renderer): this module exists so `engine: "blitz"` on
//! /screenshot stays available for comparison renders and the dual-engine
//! cross-check tests keep an oracle to diff against.
//!
//! Feed a JS-rendered HTML string to Blitz (Stylo + Taffy layout, vello_cpu
//! paint) and return a PNG. aginxbrowser's V8 path has already run the
//! page's JS and produced the final DOM; we hand that DOM to Blitz purely
//! for layout + rasterization. Blitz itself does no networking: the caller
//! pre-fetches the visible sub-resources (images, head stylesheets) over
//! HTTP and hands them to [`PrefetchedNetProvider`], which serves them
//! synchronously. Without a pre-fetch, the no-op DummyNetProvider applies
//! and upstream #636's `is_noop()` gating stops `<head>` stylesheets from
...
use crate::screenshot::{ElementRect, PrefetchedResources, RenderedScreenshot};
